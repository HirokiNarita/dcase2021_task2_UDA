...
class CenterLoss(nn.Module):
    ...
    def forward(self, x, labels=None):
        ...
        if self.training == True:
            outlier_mask = labels.clone()
            outlier_mask[outlier_mask > 0] = 1

            outlier_mask = torch.logical_not(outlier_mask).long()
            # (N, n_class, n_features)
...
class EfficientNet_b1(nn.Module):
    ...
    def forward(self, x, section_label, label=None, device='cuda:0', is_aug=True, eval=False):
        ...
        if is_aug == True:
            features, y = self.mixup(features, section_label)
        else:
            y = F.one_hot(section_label, num_classes=self.n_classes)
        centerloss_out = self.fc_block(features)
