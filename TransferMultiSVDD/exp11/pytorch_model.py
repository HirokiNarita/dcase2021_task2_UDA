...
class Autoencoder(nn.Module):
    ...
    def encoder(self, x, label,):
        x = self.enc(x)
        cl_loss, inlier_dist = self.CenterLoss(x, label)
        mean = self.fc_mean(x) # 平均
        log_var = self.fc_var(x) # 分散の対数
        return mean, log_var, cl_loss, inlier_dist
    ...
    def forward(self, input, label, device='cuda:0'):
        mean, log_var, cl_loss, inlier_dist = self.encoder(input, label)
        z = self.reparametrizaion(mean, log_var, device)

        x_hat = self.decoder(z)
    
        recons_loss = (x_hat - input).pow(2).mean(dim=1)
        kld_loss = -0.5 * torch.sum(1 + log_var - mean ** 2 - log_var.exp(), dim = 1)

        pred = recons_loss + self.beta * self.kld_weight * kld_loss + inlier_dist
        loss = recons_loss.mean() + self.beta * self.kld_weight * kld_loss.mean() + cl_loss
        return loss, pred
...
class EfficientNet_b1(nn.Module):
    def __init__(self, n_out=36, n_classes=6):
        ...
        #self.CenterLoss = CenterLoss(self.n_classes, 1280)
        # CenterLoss用のネットワーク
    ...
    def forward(self, x, section_label, label=None, device='cuda:0', is_aug=True, eval=False):
        ...
        loss, pred = self.ae(embedding, section_label)
