...
import preprocessing_crop as prep_eval
...
def make_dataloader(train_paths, machine_type):   
    ...
    transform_eval = transforms.Compose([
        prep_eval.extract_melspectrogram(eval=True)
    ])

    train_dataset = prep.DCASE_task2_Dataset(train_paths[machine_type]['train'], training=True)
    valid_source_dataset = prep_eval.DCASE_task2_Dataset(train_paths[machine_type]['valid_source'], transform=transform_eval)
    valid_target_dataset = prep_eval.DCASE_task2_Dataset(train_paths[machine_type]['valid_target'], transform=transform_eval)
...
def train_fn(data_loader, model, optimizer, epoch, device):
    ...
    for iter, sample in tqdm(enumerate(data_loader), total=len(data_loader)):
        ...
        # effnet forward
        loss, _ = model.forward(feature, section_label)
        # hook
...
def validate_fn(data_loader, model, device, get_anomaly_score=False):
    ...
    for iter, sample in tqdm(enumerate(data_loader), total=len(data_loader)):
        ...
        with torch.no_grad():
            # forward
            loss, pred = model.forward(feature, section_label)
