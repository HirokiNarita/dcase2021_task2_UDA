...
class IDNN(nn.Module):
    def __init__(self, in_size=512, out_size=128, classes=6):
        ...
        # F (encoder)
        self.enc_block1 = FC_block(in_size+classes, 128)
    ...
    def forward(self, X, section_label, alpha=1.0):
        ...
        y = X[:, :, 2]
        # network
        section_label_onehot = F.one_hot(section_label, num_classes=self.num_classes)
        # F (encoder)
        x = torch.cat([section_label_onehot, x], dim=1)
        x = self.enc_block1(x)
        x = self.enc_block2(x)
        z = self.enc_block3(x)
