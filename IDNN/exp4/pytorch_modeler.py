...
def train_fn(data_loader, model, optimizer, device):
    ...
    for iter, sample in enumerate(tqdm(data_loader)):
        # input
        raw_feature = sample['feature'].clone().detach()
        aug_feature = aug(sample['feature'].clone().detach())
        # pseudo label
        label_normal = torch.zeros(raw_feature.shape[0])
        label_anomaly = torch.ones(aug_feature.shape[0])
        # concat and shuffle
        label = torch.cat([label_normal, label_anomaly], dim=0)
        rand_idx = torch.randperm(label.size()[0])
        label = label[rand_idx]
        feature = torch.cat([raw_feature, aug_feature], dim=0)
        ...
        # propagation
        pred = model(feature, section_label)
