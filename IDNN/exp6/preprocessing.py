...
def make_freqseq(X, hop_mode=False):
    # mel_spectrogram is np.ndarray [shape=(n_mels, t)]
    mel_bins = config['param']['mel_bins']
    n_frames = config['param']['n_frames']
    total_frames = X.shape[0] - n_frames + 1
    freqseq = []
    # generate feature vectors by concatenating multiframes
    for mel_idx in range(total_frames):
        freqseq.append(X[mel_idx:mel_idx+n_frames, :])
    freqseq = np.stack(freqseq, axis=0)
...
class DCASE_task2_Dataset(torch.utils.data.Dataset):
    ...
    def __init__(self, file_list, transform=None):
        ...
        for file_name in tqdm(file_list):
            ...
            feature = make_freqseq(feature)               # [shape=(samples, n_mels, neighbor_mels)]
