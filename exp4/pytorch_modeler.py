...
def train_fn(data_loader, model, optimizer, criterion, metric, device):
    model.train()
    # hock
    ...
    for iter, sample in enumerate(tqdm(data_loader)):
        ...
        #optimizer.zero_grad()
        #with torch.cuda.amp.autocast():
        embedding_feat = model(feature)
        pred = metric(embedding_feat, section_label)
...
def validate_fn(data_loader, model, criterion, metric, device):
    model.eval()
    # hock
    # def hook(module, input, output):
    #     #print(output.shape)
...
def run_training(model, dataloaders_dict, writer, optimizer, metric):
    device = torch.device("cuda:0" if torch.cuda.is_available() else 'cpu')
    print("use:", device)
    model.to(device)
    metric.to(device)
...
import utils
def estimate_fisher(model, data_loader, sample_size, metric, batch_size=32):
    # Init
    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    model.to(device)
