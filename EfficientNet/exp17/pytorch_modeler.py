...
def make_dataloader(train_paths, machine_type):   
    ...
    valid_source_loader = torch.utils.data.DataLoader(
        ...
        batch_size=config['param']['batch_size'],  # 1バッチにつき一つのスペクトログラム
        shuffle=False,
        )
...
def validate_fn(data_loader, model, device, get_anomaly_score=False):
    ...
    for iter, sample in tqdm(enumerate(data_loader), total=len(data_loader)):
        ...
        output_dict['section_label'].append(sample['section_label'])
        output_dict['domain_label'].append(sample['domain_label'])
        output_dict['wav_name']+=sample['wav_name']
        output_dict['pred'].append(pred)
    
    # concat for output
    output_dict['loss'] = output_dict['loss'] / len(data_loader)
    output_dict['label'] = torch.cat(output_dict['label']).detach().numpy().copy()
    output_dict['section_label'] = torch.cat(output_dict['section_label']).detach().numpy().copy()
    output_dict['domain_label'] = torch.cat(output_dict['domain_label']).detach().numpy().copy()
    output_dict['pred'] = torch.cat(output_dict['pred']).to('cpu').detach().numpy().copy()
    return output_dict
