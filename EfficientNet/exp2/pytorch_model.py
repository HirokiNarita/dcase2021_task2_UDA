...
class CenterLoss(nn.Module):
    ...
    def forward(self, x, labels=None):
        ...
        # 正常 == Entropy 小
        inlier_dist = (inlier_x-inlier_center).pow(2)#.pow(1)
        # outlier用のcenter行列作成
        # 異常 == Entropy 大
        if self.training == True:
            ...
            outlier_dist = (outlier_x-self.centers.unsqueeze(0)).pow(2).pow(-1)
            loss = (inlier_dist.mean() + self.k * outlier_dist.mean()) / x.shape[0]
        else:
            loss = inlier_dist.mean() / x.shape[0]

        inlier_dist = inlier_dist.mean(-1) / x.shape[0]
