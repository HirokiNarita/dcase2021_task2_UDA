...
class VAE(nn.Module):
    def __init__(self, in_features, mid_features=512):
        ...
        # enc
        self.fc1 = nn.Linear(in_features+self.num_classes, mid_features)
    ...
    # デコーダー
    def decoder(self, z):
        y = self.act(self.bn2(self.fc2(z)))
        y = self.fc3(y)
        return y
    ...
    def forward(self, x, label, device):
        ...
        mean, log_var = self.encoder(x_cat)
...
class EfficientNet_b1(nn.Module):
    ...
    def forward(self, x, section_label):
        ...
        lower_bound , _, x_hat = self.vae(embedding, section_label, device='cuda:0')
