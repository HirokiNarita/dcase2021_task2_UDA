...
from metric_module import ArcMarginProduct
...
class EfficientNet_b1(nn.Module):
    def __init__(self, n_out=36, n_classes=6):
        ...
        self.fc_block0 = nn.Sequential(
            nn.Linear(1280+n_classes, 1280+n_classes, bias=False),
            nn.BatchNorm1d(1280+n_classes),
            nn.SiLU(),
            nn.Linear(1280+n_classes, 1280+n_classes, bias=False),
            )

        self.arc_face = ArcMarginProduct(in_features=1280+n_classes,
                                         out_features=2)
    ...
    def forward(self, x, section_label, label=None, device='cuda:0'):
        ...
        pseudo_x = None
        onehot_classes = F.one_hot(section_label, num_classes=6)
        if self.training == True:
            pseudo_x = self.spec_cutmix(x)
            pseudo_N = pseudo_x.shape[0]
            x = torch.cat([x, pseudo_x], dim=0)
            label = torch.cat([torch.zeros((batch_size,)), torch.ones((batch_size,))], dim=0).long().to(device)
            onehot_classes = torch.cat([onehot_classes, onehot_classes], dim=0)
        ...
        embedding_cat = torch.cat([embedding, onehot_classes], dim=1)
        embedding_cat = self.fc_block0(embedding_cat)

        pred = self.arc_face(embedding_cat, label)
        loss = self.forcal_loss(pred, label)
