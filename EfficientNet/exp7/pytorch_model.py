...
class Binary_Classifier(nn.Module):
    def __init__(self, num_feature=2, normal_label=[0, 7, 14, 21, 28, 35]):
    ...
    def replace_label(self, labels, outlier_num=1):
        # 7の倍数を置き換え（もっと適切な方法があるはず）
        # 他の値を99で置き換え
        #labels = labels.to('cpu').detach().clone()
        labels = torch.where((labels % 7) == 0 , labels, outlier_num)
        for i in range(len(self.normal_label)):
            labels[labels == self.normal_label[i]] = 0
...
class CenterLossNet(nn.Module):
    def __init__(self, in_features, out_features=6, mid_features=None):
        ...
        self.out = Binary_Classifier(num_feature=mid_features)

    def forward(self, x, section_label):
        x = self.fc_in(x)
        x = self.fc_blocks(x)

        loss, inlier_dist = self.out(x, section_label)
        return loss, inlier_dist
