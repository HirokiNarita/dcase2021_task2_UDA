...
class CenterLoss(nn.Module):
    ...
    def forward(self, x, pseudo_x=None, labels=None):
        ...
        if self.training == True:
            ...
            outlier_dist = torch.tanh(outlier_dist.clamp(min=1e-12, max=1e+12).sum(dim=1))      # (N, 1, n_features)
...
class EfficientNet_b1(nn.Module):
    ...
    def forward(self, x, section_label):
        ...
        if self.training == True:
            center_loss, pred = self.CenterLoss(centerloss_out[:pseudo_N],
                                                pseudo_x=centerloss_out[pseudo_N:],
                                                labels=section_label)
        else:
            center_loss, pred = self.CenterLoss(centerloss_out,
                                                pseudo_x=None,
                                                labels=section_label)            
