...
def train_fn(data_loader, model, optimizer, epoch, device):
    ...
    centers = model.CenterLoss.centers[:].clone().detach()
    model.gaussian_density_estimation.set_param(embedding_feats,
                                                section_labels,
                                                centers=centers)
    # concat for output
...
def validate_fn(data_loader, model, device, get_anomaly_score=False):
    ...
    for iter, sample in tqdm(enumerate(data_loader), total=len(data_loader)):
        ...
        with torch.no_grad():
            # forward
            loss, _, pred  = model.forward(feature, section_label)
