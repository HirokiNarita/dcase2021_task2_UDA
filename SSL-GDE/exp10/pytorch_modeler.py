...
def make_dataloader(train_paths, machine_type):   
    ...
    train_loader = torch.utils.data.DataLoader(
        ...
        num_workers = int(os.cpu_count() / 2) - 1,
        )
...
def set_oe_model(data_loader, model, optimizer, epoch, device):
    ...
    # GDE
    embedding_feats = torch.cat(embedding_feats)
    embedding_feats = embedding_feats.cpu()
    #print(embedding_feats.shape)
    section_labels = torch.cat(section_labels)
    section_labels = section_labels.cpu()
    lof_models = []
    for section in range(6):
        idx = torch.where(section_labels == section)[0]
        #print(idx)
        X_ = embedding_feats[idx].cpu().detach().numpy().copy()
        #print(X_.shape)
        lof_model = lof.LOF(n_jobs=-1, n_neighbors=4)
        lof_model.fit(X_)
    ...
    # concat for output
    output_dict['loss'] = output_dict['loss'] / len(data_loader)
    output_dict['oe_models'] = lof_models

    return output_dict

def validate_fn(data_loader, model, device, oe_models, get_anomaly_score=False):
    model.eval()
    # init
    output_dict = {
        'loss': 0,
    ...
    for iter, sample in tqdm(enumerate(data_loader), total=len(data_loader)):
        ...
        with torch.no_grad():
            # forward
            loss, embedding_feats, pred = model.forward(feature, section_label, label=label, is_aug=False, eval=True)
            embedding_feats = embedding_feats.cpu()
            section_label = section_label.cpu()

            X_ = embedding_feats.cpu().detach().numpy().copy()
            pred = oe_models[section_label[0]].decision_function(X_)
    ...
    output_dict['pred'] = np.stack(output_dict['pred'])
    return output_dict
    ...
    for epoch in range(n_epochs):
        ...
        if epoch == n_epochs:
            ...
            output_src = validate_fn(
                ...
                oe_models = output_oe_model['oe_models']
                )
