...
def make_dataloader(train_paths, machine_type):   
    ...
    transform_set_gde = transforms.Compose([
        prep.extract_melspectrogram(mode='set_gde')
    ])    

    train_dataset = prep.DCASE_task2_Dataset(train_paths[machine_type]['train'], transform=transform_tr)
    train_set_gde_dataset = prep.DCASE_task2_Dataset(train_paths[machine_type]['set_gde'], transform=transform_set_gde)
    ...
    train_set_gde_loader = torch.utils.data.DataLoader(
        dataset=train_set_gde_dataset,
        batch_size=32,
        shuffle=False,
        num_workers = os.cpu_count() - 1,
        )
...
#        num_workers = os.cpu_count(),
    ...
    dataloaders_dict = {"train": train_loader, "valid_source": valid_source_loader, "valid_target": valid_target_loader, 'train_set_gde': train_set_gde_loader}
...
def set_gde(data_loader, model, optimizer, epoch, device):
    # tmp_deep_feat = []
    # def hook(module, input, output):
    #     #print(output.shape)
    #     output = F.adaptive_avg_pool2d(output, 1).squeeze()
    ...
    for iter, sample in tqdm(enumerate(data_loader), total=len(data_loader)):
        ...
        n_samp, n_seq = feature.shape[0], feature.shape[1]
        feature = feature.flatten(0,1).to(device)
        
        section_label = [torch.full(size=(n_seq,),fill_value=sample['section_label'][i]) for i in range(n_samp)]
        section_label = torch.cat(section_label).to(device)

        with torch.no_grad():
            loss, embedding, _ = model.forward(feature, section_label, is_aug=False, eval=False)
...
def validate_fn(data_loader, model, device, get_anomaly_score=False):
    ...
    for iter, sample in tqdm(enumerate(data_loader), total=len(data_loader)):
        ...
        with torch.no_grad():
            # forward
            loss, _, pred  = model.forward(feature, section_label, label=label, is_aug=False, eval=True)
...
def run_training(model, dataloaders_dict, writer, optimizer, scheduler):
    ...
    for epoch in range(n_epochs):
        ...
        if epoch == n_epochs:
            set_gde(
                data_loader=dataloaders_dict['train_set_gde'],
                model=model,
                optimizer=optimizer,
                epoch=epoch,
                device=device,
